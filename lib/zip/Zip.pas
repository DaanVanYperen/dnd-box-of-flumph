...
{$WARNINGS OFF} //hides 'Unsafe Type' compiler warnings
  ...
  TFileInfo = packed record
    ...
    Filename      : string;   //ANSI format
    ExtraField    : string;
    FileComment   : string;   //ANSI format
  ...
  private
    ...
    function OemStrToAnsiStr(const s: string): string;
    function AnsiStrToOemStr(const s: string): string;
...
begin
  ...
  result := GetVolumeInformation(pchar(DrivePath),
               VolName, sizeof(VolName), nil, dummy1, dummy2, nil, 0 );
...
begin
  ...
  try
    ...
    ZipParams.ZipFilename := pchar(fFilename);
    ZipParams.TempPath := pchar(fTempPath);
    OemComment := AnsiStrToOemStr(fEocInfo.ZipComment);
    if (OemComment <> '') then
      ZipParams.FileComment := pchar(OemComment);
    if IsAdd and (fPassword <> '') then
      ZipParams.ZipPassword := pchar(fPassword);
